...
def get_dynDB_client():
    ...
    if IS_LOCAL_DYNDB:
        return boto3.client('dynamodb', endpoint_url="http://dynamodb-local:8000", region_name="local")
    else:
...
def get_dynDB_resource():
    IS_LOCAL_DYNDB = os.getenv('IS_LOCAL_DYNDB')
    if IS_LOCAL_DYNDB:
        return boto3.resource('dynamodb', endpoint_url="http://dynamodb-local:8000", region_name="local")
    else:
...
# the 'hello' endpoint just shows that the webserver is working, and has no DynamoDB dependencies
...
@app.route('/url-info/<slug>', methods =['GET'])
def url_info(slug):
...
if __name__ == '__main__':
    ...
    if os.getenv('IS_LOCAL_DYNDB') == None:
